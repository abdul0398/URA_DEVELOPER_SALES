...
export default function MonthDetail() {
    const { includeEC, selectedMonth, setSelectedMonth, setIncludeEC } = useContext(MyContext);
    ...
    const handleCheckboxChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (val === "All") {
            ...
            setIncludeEC(isChecked ? IncludeEC.EC : includeEC === IncludeEC.EC ? IncludeEC.NonEC : IncludeEC.All);
        } else {
            setIncludeEC(isChecked ? IncludeEC.NonEC : includeEC === IncludeEC.NonEC ? IncludeEC.EC : IncludeEC.All);
        }
    };

    let listings = data[selectedMonth].listings
    listings = listings.filter((item: any) => item.developerSales[0].launchedInMonth > 0)

    if (includeEC != 'All') {
        console.log(includeEC);
        listings = listings.filter((item: any) => item.propertyType == (includeEC == "EC" ? "Exec Condo" : "Non-Landed"))
    }

    listings.sort((a: any, b: any) => b.developerSales[0].launchedInMonth - a.developerSales[0].launchedInMonth)
    ...
    const Row: React.FC<ListChildComponentProps> = ({ index, style }) => {
        ...
        return (
            <div key={index} style={style} className="h-14 grid gap-1 grid-cols-[25%_8%_8%_8%_10%_15%_10%_10%] border-b transition duration-300 ease-in-out hover:bg-neutral-100 dark:border-neutral-500 dark:hover:bg-neutral-600">
                ...
                <div className="px-1 text-xs">{data.developerSales[0].launchedToDate}</div>
                <div className="px-1 text-xs">{data.developerSales[0].soldToDate}</div>
                <div className="px-1 text-xs">{data.developerSales[0].unitsAvail - data.developerSales[0].soldToDate}</div>
    ...
    return (
        <div className="w-1/2 border">
            <h1 className="text-center h-[7%]">Unit Table</h1>
            <div className="bg-white w-full h-[93%] px-2">
